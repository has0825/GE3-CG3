...
#include "MathTypes.h" // ★ DataTypesの前にインクルード
...
struct D3DResourceLeakChecker {
...

	{
		...
		// ★ 修正点 1: IID_PPVARGS -> IID_PPV_ARGS
...

{
	...
	// (ImGuiのコードは省略しますが、SRVヒープはImGuiにも必要です)
	...
	while (!winApp->IsEndRequested()) {
		...
		if (input->IsKeyPressed(DIK_W)) {
			...
			move.z += kMoveSpeed; // 奥へ

		}

		if (input->IsKeyPressed(DIK_S)) {

			move.z -= kMoveSpeed; // 手前へ

		}

		if (input->IsKeyPressed(DIK_A)) {

			move.x -= kMoveSpeed; // 左へ

		}

		if (input->IsKeyPressed(DIK_D)) {

			move.x += kMoveSpeed; // 右へ
		...
		// viewMatrix = Inverse(MakeAffineMatrix(cameraTransform.scale, cameraTransform.rotate, cameraTransform.translate));
